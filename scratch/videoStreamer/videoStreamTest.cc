...
main (int argc, char *argv[])
{
  ...
  if (CASE == 1)
  {
    ...
    VideoStreamClientHelper videoClient (interfaces.GetAddress (0), 6969);
    ApplicationContainer clientApp = videoClient.Install (nodes.Get (1));
    clientApp.Start (Seconds (0.5));
    clientApp.Stop (Seconds (100.0));

    VideoStreamServerHelper videoServer (6969);
    ...
    // Add the following statements for xml generation for flow monitor
    flowMonitor -> SerializeToXmlFile("Case_1_flowmonitor.xml",true,true);
   // Print throughput for each flow
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator it = stats.begin (); it != stats.end (); ++it)
    {
        ...
        std::cout << "Flow from " << flow.sourceAddress << " to " << flow.destinationAddress << std::endl;
        std::cout << "  Tx Packets: " << it->second.txPackets << std::endl;
        std::cout << "  Rx Packets: " << it->second.rxPackets << std::endl;
        std::cout << "  Throughput: " << (it->second.rxBytes * 8.0) / 100.0 / 1024 / 1024 << " Mbps" << std::endl;
        std::cout << "  Average Delay: " << (it->second.delaySum / it->second.rxPackets).GetSeconds() * 1024 << " ms" << std::endl;
    }
    Simulator::Destroy ();
  }
  ...
  else if (CASE == 2)
  {
    ...
    address1.Assign (d0d2);

    //Ipv4InterfaceContainer interfaces = address.Assign (devices);
    Ipv4InterfaceContainer i0i1 = address.Assign (d0d1);
    Ipv4InterfaceContainer i0i2 = address1.Assign (d0d2);

    // Packet sink applications for clients
  ...
  clientSinkApp1.Start(Seconds(0.0));  // Start before video clients send packets
  clientSinkApp1.Stop(Seconds(100.0));  // Stop after simulation ends

  ApplicationContainer clientSinkApp2 = sinkHelper.Install(nodes.Get(2));
  clientSinkApp2.Start(Seconds(0.0));  // Start before video clients send packets
  clientSinkApp2.Stop(Seconds(100.0));  // Stop after simulation ends

    VideoStreamClientHelper videoClient1 (i0i1.GetAddress (0), 6969);
    ApplicationContainer clientApp1 = videoClient1.Install (nodes.Get (1));
    clientApp1.Start (Seconds (1.0));
    clientApp1.Stop (Seconds (100.0));

    VideoStreamClientHelper videoClient2 (i0i2.GetAddress (0), 6969);
    ...
    Simulator::Run ();
    // Add the following statements for xml generation for flow monitor
    flowMonitor -> SerializeToXmlFile("Case_2_flowmonitor.xml",true,true);
    // Print throughput for each flow
    flowMonitor->CheckForLostPackets();
    Ptr<Ipv4FlowClassifier> classifier = DynamicCast<Ipv4FlowClassifier>(flowHelper.GetClassifier ());
    FlowMonitor::FlowStatsContainer stats = flowMonitor->GetFlowStats ();
